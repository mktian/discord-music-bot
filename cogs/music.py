...
class Music(commands.Cog):
    ...
    async def p(self, ctx, *args):
        ...
        if len(songs) < 1:
            embed = discord.Embed(
                description='You search like lady eh? I went to the hill and went down and couldn\'t find your video. Branched.'
            )
            await ctx.send(embed=embed)
            # await ctx.send("You search like lady eh? I went to the hill and went down and couldn't find your video. Branched.")
        elif len(songs) == 1:
            song = songs[0]
            embed = discord.Embed(
    ...
    async def q(self, ctx):
        ...
        if retval != "":
            ...
            embed = discord.Embed(
                title='Current songs in queue:',
                description='Bloody NO! There are no bloody songs in the queue, branched.'
            )
            await ctx.send(embed=embed)
    ...
    @commands.command(name="stop", help="Stops and clears queue", aliases=['st'])
